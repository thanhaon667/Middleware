...
async function upsertConfig(clientName: string, data: Record<string, any>) {
...
async function getValidAccessToken(config: any) {
  if (config.googleAccessToken && config.tokenExpiresAt && new Date(config.tokenExpiresAt).getTime() > Date.now() + 60_000) {
...
async function fetchExistingOrderIds(sheetId: string, tab: string, accessToken: string) {
  const idRange = `${tab}!C:C`;
  const response = await axios.get(
    `${GOOGLE_SHEETS_BASE}/${sheetId}/values/${encodeURIComponent(idRange)}`,
    {
      headers: {
...
function mapOrderRow(order: any) {
...
export default {
  ...
  async createAuthUrl(clientName: string, sheetUrl: string, sheetTab = 'Orders') {
    ...
    if (!sheetId) throw new Error('Invalid sheetUrl');
  ...
  async syncClientOrders(clientName: string) {
    ...
    const newOrders = orders.filter((o: any) => !!o.orderId && !existingIds.has(String(o.orderId)));
    ...
    await axios.post(
      `${GOOGLE_SHEETS_BASE}/${config.sheetId}/values/${encodeURIComponent(range)}:append`,
      {
        values: rows,
        majorDimension: 'ROWS'
      },
