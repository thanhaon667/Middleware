 ...
 */
async function getCredentialByClientName(clientName: string) {
...
function buildSapoPrivateAppUrl(sapoCred: any, path: string) {
  const shopDomain = normalizeSapoShopDomain(sapoCred?.sapoShopDomain);
  const normalizedPath = path.startsWith('/') ? path : `/${path}`;
  return `https://${encodeURIComponent(String(sapoCred?.sapoApiKey || ''))}:${encodeURIComponent(String(sapoCred?.sapoApiSecret || ''))}@${shopDomain}${normalizedPath}`;
}
...
async function addSapoOrderLogById(localId: number, step: string, message: string, isError: boolean = false) {
  try {
 ...
 */
async function getMisaTokenByCredential(cred: any): Promise<string> {
 ...
 */
function mapZeekStatusToEnum(code: number, desc: string): string {
  ...
  switch (code) {
    ...
    default:
      ...
      if (lowerDesc.includes('đã giao hàng') || lowerDesc.includes('giao thành công')) return 'COMPLETED';
      if (lowerDesc.includes('thất bại') || lowerDesc.includes('không giao được') || lowerDesc.includes('hủy')) return 'FAILED';
      if (lowerDesc.includes('đang giao')) return 'IN_DELIVERY';
      if (lowerDesc.includes('đang lấy hàng') || lowerDesc.includes('lấy hàng')) return 'PICKING_UP';
 ...
 */
async function getSapoCredentialForClient(clientName: string) {
 ...
 */
async function updateSapoOrderTags(sapoOrderId: number, sapoCred: any, newTag: string) {
  ...
  try {
    ...
    const getResp = await axios.get(orderUrl, { headers: { 'Content-Type': 'application/json' } });
...
export default {
  async receive(ctx) {
    ...
    try {
      ...
      const deliveryID = zeekData.order_id || (zeekData.tasks?.[0]?.id) || null;
      const trackURL = null;
      ...
      if (statusEnum === 'COMPLETED') newOrderStatus = 'completed';
      else if (statusEnum === 'FAILED') newOrderStatus = 'failed';
      else if (statusEnum === 'IN_DELIVERY' || statusEnum === 'PICKING_UP') newOrderStatus = 'processing';
      ...
      if (platform === 'sapo') {
        ...
        try {
          ...
          if (order.platform === 'sapo') {
            ...
            else if (statusEnum === 'PENDING') newStatus = 'new';
  ...
  async testSapoConnection(ctx) {
    const clientName = String(ctx.params?.clientName || '').trim();
    const limit = Math.max(1, Math.min(Number(ctx.query?.limit || 1) || 1, 10));
    ...
    try {
      ...
      ctx.body = {
        ...
        sampleOrders: orders.slice(0, limit).map((order: any) => ({
          id: order.id,
          name: order.name,
