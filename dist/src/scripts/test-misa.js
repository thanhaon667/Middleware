 ...
 */
async function writeStepLog(clientDocId, step, details, isError = false) {
    try {
        if (!clientDocId) {
 ...
 */
async function addOrderLog(clientName, orderId, step, message, isError = false) {
    try {
...
async function withMisaAuthRetry(requestFn) {
    var _a;
 ...
 */
async function fetchMisaOrders() {
    var _a, _b, _c;
    ...
    try {
        const response = await withMisaAuthRetry((token) => axios_1.default.get(ordersUrl, {
        ...
        let orders = [];
        if (Array.isArray(response.data) && ((_b = (_a = response.data[0]) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.success)) {
            orders = response.data[0].data.data;
        }
        else if ((_c = response.data) === null || _c === void 0 ? void 0 : _c.success) {
            orders = response.data.data;
        }
        if (!orders.length)
            console.warn('⚠️ No orders returned from MISA');
        else
...
async function getOrCreateClient(cred) {
    ...
    const clientAppId = ((_b = cred.clientMerchantId) === null || _b === void 0 ? void 0 : _b.trim()) || ((_c = cred.misaClientId) === null || _c === void 0 ? void 0 : _c.trim()) || clientName;
...
async function sendToZeek(order, clientName) {
    ...
    try {
        ...
        if (response.data.error === 0) {
            const zeekOrderId = (_a = response.data.data) === null || _a === void 0 ? void 0 : _a.order_id;
 ...
 */
async function updateOrderAfterSend(order, clientName, zeekOrderId) {
...
async function processCredential(cred) {
    var _a;
    let client = null;
    const clientName = ((_a = cred.clientName) === null || _a === void 0 ? void 0 : _a.trim()) || 'UnknownClient';
