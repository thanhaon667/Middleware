...
exports.default = {
    async sendOrder(order, connection) {
        try {
            const payload = this.mapOrderToSmartMinds(order, connection.config);
            const response = await axios_1.default.post(`${connection.apiUrl}/order/takeaway/create`, payload, {
                headers: {
    ...
    mapOrderToSmartMinds(order, config) {
        ...
        if (misa.sale_order_product_mappings && misa.sale_order_product_mappings.length) {
            itemWeight = misa.sale_order_product_mappings.reduce((sum, item) => {
